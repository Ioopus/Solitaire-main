    ...
    public class Solitaire : MonoBehaviour
    {
        ...
        public int gameScore = 0; // Current game's score
        ...
        void Start()
        {
            ...
            if (!vegasCumulative)
            {
                score = -52; // Reset score at the start of each new game if Vegas cumulative option is off
            }
            else
            {
                score = PlayerPrefs.GetInt("VegasScore", -52); // Load score if Vegas cumulative option is on
                gameScore = score; // Set gameScore to score at the start of each game
            }
            PlayCards();
        }
...
{
    int dropdownOption = PlayerPrefs.GetInt("DropdownOption", 0);
    ...
    switch (dropdownOption)
    {
        ...
        case 2: // Draw3 option
            ...
            drawThree = true;
            break;
        default: // Default to Regular option
            vegasCumulative = false;
            drawThree = false;
            break;
    }

    // Check if deckTrips is empty
    if (deckTrips.Count == 0)
    {
    ...
    int drawCount = 1; // Default draw count is 1
    if (drawThree && deckLocation < trips)
    {
        drawCount = Mathf.Min(3, trips - deckLocation); // Ensure draw count doesn't exceed available trips
    }
    ...
    // Update score based on card draw
    gameScore += drawCount * 5;

    // Update the score based on the selected options
    if (vegasCumulative)
    {
        score += drawCount * 5;
        PlayerPrefs.SetInt("VegasScore", score); // Save score if Vegas cumulative option is on
    }
    ...
    print("Score: " + gameScore);
        ...
        public void RestackTopDeck()
        {
            while (discardPile.Count > 0)
            {
                ...
                string lastCard = discardPile[lastCardIndex];
